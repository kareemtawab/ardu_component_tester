...
//Voltage offset of Ã‚ÂµCs analog comparator (in mV): -50 up to 50
#define COMPARATOR_OFFSET     15
//Capacitance of the probe tracks of the PCB and the Ã‚ÂµC (in pF)
#define CAP_PCB               42
...
typedef struct
{
  ...
  unsigned int                RiL;               //Internal pin resistance of Ã‚ÂµC in low mode (0.1 Ohms) 
  unsigned int                RiH;               //Internal pin resistance of Ã‚ÂµC in high mode (0.1 Ohms) 
...
typedef struct
{
  ...
  //U_BE voltage
  unsigned int                I_CE0;             //Leakage current (in Ã‚ÂµA) 
} BJT_Type;
  ...
  */
  ...
  /*
     ...
     Other possibilities:
      - diode or resistor
  */
  if (U_Rl > 490)                                // > 700Ã‚ÂµA (was 92mV/130Ã‚ÂµA)
  {
...
unsigned long Get_hFE_C(byte Type)
{
  ...
  unsigned int                Ri;                //Internal resistance of Ã‚ÂµC
  /*
  ...
  */
  ...
  if (U_R_b < 10)                                //I_b < 14Ã‚ÂµA -> Darlington 
  {
  ...
  /*
     ...
     Solution:
      ...
      - For a MOSFET pretection diode we have to make sure that the MOSFET
        ...
        channel FETs.
      - Take care about the internal voltage drop of the Ã‚ÂµC at the cathode
      ...
      - Filter out caps by checking the voltage before and after measurement
        with Rh. In 15ms a 22Ã‚ÂµF cap would be charged from 0 to 7mV, a larger
      ...
      - Rl drives a current of about 7mA. That's not the best current for
        measuring Vf. The current for Rh is about 10.6Ã‚ÂµA.
        Most DMMs use 1mA.
...
void CheckBJTorEnhModeMOSFET(byte BJT_Type, unsigned int U_Rl)
{
  ...
  {
    BJT_Level = 2557;                            //Voltage across base resistor (5.44Ã‚ÂµA)
  ...
  {
    BJT_Level = 977;                             //Voltage across base resistor (2.1Ã‚ÂµA) 
  ...
  if (U_R_b > BJT_Level)                         //U_R_b exceeds minimum level of BJT
  {
    ...
    */
    ...
    Check.Type = BJT_Type;
    //Leakage current
    I_CE0 = GetLeakageCurrent();                 //Get leakage current (in Ã‚ÂµA)
    /*
  ...
  */
  ...
  while (Mode > 0)
  {
    ...
    while (Counter < 100)
    {
      ...
      ADC_DDR = Probes.ADC_2;                    //Pull-down probe-2 directly
      R_PORT = Probes.Rl_1;
      //Start ADC conversion, ADC performs S&H after 1.5 ADC cycles (12Ã‚Âµs)
      ADCSRA |= (1 << ADSC);                     //Start conversion
      //Wait 20Ã‚Âµs to allow the ADC to do it's job
      waitus(20);
      //Stop pulse
      R_PORT = 0;
      ADC_DDR = Probes.ADC_2 | Probes.ADC_1;
      //Get ADC reading (about 100Ã‚Âµs)
      while (ADCSRA & (1 << ADSC));              //Wait until conversion is done 
      Value += ADCW;                             //Add ADC reading 
      //Wait
      waitus(900);
...
void CheckResistor(void)
{
  ...
  /*
     Resistor measurement
      ...
      - For low resistance consider the internal resistors of the Ã‚ÂµC
        for pulling up/down.
  ...
  */
  ...
  U_Ri_L = ReadU_5ms(Probes.Pin_2);              //Get voltage at internal R of Ã‚ÂµC 
  ...
  */
  ...
  if (U_Rh_L <= 20)
  {
    ...
    R_DDR = Probes.Rl_2;                         //Pull down probe-2 via Rl 
    U_Ri_H = ReadU_5ms(Probes.Pin_1);            //Get voltage at internal R of Ã‚ÂµC 
...
//Measure cap >4.7Ã‚ÂµF between two probe pins
byte LargeCap(Capacitor_Type *Cap)
{
  ...
  */
  if (Flag == 3)
  {
    ...
    Cap->Raw = Raw;
    Cap->Value = Value;                          //Max. 4.3*10^6nF or 100*10^3Ã‚ÂµF
  }
  return Flag;
}
...
byte SmallCap(Capacitor_Type *Cap)
{
  ...
  /*
     ...
     use the Ã‚ÂµCs internal 16-bit counter and analog comparator. The counter
  ...
  */
  ...
  if (Flag == 3)
  {
    ...
    /*
       ...
       bandgap reference if C is 100nF up to 20Ã‚ÂµF. The minimum of 100nF
        ...
        ((Scale == -9) && (Value <= 20000)))
    {
      ...
      */
      ...
      /*
         ...
         the voltages of the cap and the bandgap reference. Since the Ã‚ÂµC
         has an internal voltage drop for the bandgap reference the
         Ã‚ÂµC used actually U_bandgap - U_offset. We get that offset by
...
void MeasureCap(byte Probe1, byte Probe2, byte ID)
{
  ...
  */
  ...
  if (Check.Diodes == 0)
  {
    ...
    if (Check.Found == COMP_RESISTOR)
    {
      //Report capacitor for large C (> 4.3Ã‚ÂµF)
...
byte MeasureInductance(uint32_t *Time, byte Mode)
{
  ...
  */
  ...
  {
    Test = (CPU_FREQ / 1000000);                 //Cycles per Ã‚Âµs
    //Change probes: Gnd -- Rl -- probe-2 / probe-1 -- Vcc
    ADC_PORT = Probes.ADC_1;                     //Pull up probe-1 directly
    /*
       Delay timer by about 3-4Ã‚Âµs to skip capacitive effects of large inductors
        - a single loop needs 4 cycles, the last loop run just 3
        - cycles burnt: <MCU cycles per Ã‚Âµs> * 4 - 1
    */
    while (Test > 0)
    {
      Test--;
  ...
  //Convert counter (MCU cycles) to time (in Ã‚Âµs)
  if (Counter > 0)
  {
    Counter += (CPU_FREQ / 2000000);             //Add half of cycles for rounding 
    Counter /= (CPU_FREQ / 1000000);             //Divide by frequeny and scale to Ã‚Âµs 
  }
...
byte MeasureInductor(Resistor_Type *Resistor)
{
  ...
  */
  ...
  if (Test == 3)
  {
    ...
    */
    Scale = -6;                                  //Ã‚ÂµH by default
    Value = Time1;                               //t_stop 
    Value *= Factor;                             // * factor (Ã‚Âµs * 10^-3) 
...
void ShowDiode(void)
{
  ...
  */
  ...
  if (D2 == NULL)                                //Single diode 
  {
    ...
    I_leak = GetLeakageCurrent();                //Get current (in Ã‚ÂµA) 
  ...
  while (Counter < Check.Diodes)                 //Check all diodes
  {
    ...
    if ((Diode->A == A_Pin) && (Diode->C == C_Pin))
    {
      ...
      /*
         ...
         measurements for a virtual test current. Low test current is 10Ã‚ÂµA
    ...
    while (Counter <= 5)
    {
      ...
      switch (Test)
      {
        ...
        case 2:                                  //Un-short probes 
          ...
          break;
        case 3:                                  //Internal resistance of Ã‚ÂµC in pull-down mode 
          ...
          RiL_Counter += 3;
          break;
        case 4:                                  //Internal resistance of Ã‚ÂµC in pull-up mode 
